...
async function fetchYouTubeData(videoId) {
  ...
  if (titleMatch) {
    ...
    try { title = JSON.parse(`"${title}"`) } catch (e) {} // unicode unescape
  }
  ...
  try {
    ...
    if (captionData && captionData.length > 0) {
      ...
      if (track?.baseUrl) {
        ...
        // XML에서 타임스탬프 + 텍스트 추출
        const textMatches = [...captionXml.matchAll(/<text start="([\d.]+)" dur="([\d.]+)"[^>]*>(.*?)<\/text>/gs)]
  ...
  // 자막이 없는 경우 innertube API로 재시도
  if (!transcript) {
    try {
      console.log('[analyze-youtube-shorts] Trying innertube API for transcript...')
      ...
      if (innertubeResponse.ok) {
        ...
        if (segments?.length) {
          ...
          transcript = timestampedTranscript.map(t => t.text).join(' ')
          console.log('[analyze-youtube-shorts] Innertube transcript segments:', timestampedTranscript.length)
        }
      }
    } catch (e) {
      console.log('[analyze-youtube-shorts] Innertube API failed:', e.message)
    }
  }

  return { title, description, duration, transcript, timestampedTranscript, videoId, captionLang }
}
