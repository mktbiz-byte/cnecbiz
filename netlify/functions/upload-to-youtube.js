...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (uploadId) {
      ...
      try {
        if (uploadRecord) {
          await syncSnsUrlToSource(uploadRecord.source_type, uploadRecord.source_id, videoUrlResult)
        }
      } catch (syncErr) {
        console.log('[upload-to-youtube] sns_upload_url sync-back error:', syncErr.message)
      }
    }
