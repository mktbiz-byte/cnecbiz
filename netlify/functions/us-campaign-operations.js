  ...
  try {
    ...
    switch (action) {
      ...
      case 'update_campaign': {
        // updated_at, 일본어 필드 등 확실히 US DB에 없는 필드 제거
        ...
        while (retries < 5) {
          ...
          if (result.error && result.error.message && result.error.message.includes('column')) {
            ...
            if (match) {
              ...
              console.log(`[US API] Removing missing column '${badCol}' and retrying`)
        ...
        console.log('[US API] update_campaign final fields:', Object.keys(updateData))
        break
      }
