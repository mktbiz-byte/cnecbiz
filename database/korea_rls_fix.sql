-- ============================================================
-- 한국 사이트 RLS (Row Level Security) 수정 SQL
-- ============================================================
--
-- 문제: 한국 DB는 프론트엔드에서 anon key로 접속하며,
...
--
-- 실행 위치: Supabase Korea 프로젝트 > SQL Editor
-- ============================================================
...
-- ============================================================
-- 4. campaign_participants 테이블 (존재하는 경우)
-- ============================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'campaign_participants' AND table_schema = 'public') THEN
        ...
        BEGIN
            FOR pol IN
                SELECT policyname FROM pg_policies WHERE tablename = 'campaign_participants'
            LOOP
                EXECUTE format('DROP POLICY IF EXISTS %I ON campaign_participants', pol.policyname);
            END LOOP;
        END;

        EXECUTE 'CREATE POLICY "campaign_participants_select_all" ON campaign_participants FOR SELECT TO anon, authenticated USING (true)';
        EXECUTE 'CREATE POLICY "campaign_participants_insert_all" ON campaign_participants FOR INSERT TO anon, authenticated WITH CHECK (true)';
        EXECUTE 'CREATE POLICY "campaign_participants_update_all" ON campaign_participants FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true)';
        EXECUTE 'CREATE POLICY "campaign_participants_delete_all" ON campaign_participants FOR DELETE TO anon, authenticated USING (true)';
    END IF;
END $$;


-- ============================================================
-- 5. video_submissions 테이블 (존재하는 경우)
-- ============================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'video_submissions' AND table_schema = 'public') THEN
        ...
        BEGIN
            ...
            LOOP
                EXECUTE format('DROP POLICY IF EXISTS %I ON video_submissions', pol.policyname);
            END LOOP;
        END;

        EXECUTE 'CREATE POLICY "video_submissions_select_all" ON video_submissions FOR SELECT TO anon, authenticated USING (true)';
        EXECUTE 'CREATE POLICY "video_submissions_insert_all" ON video_submissions FOR INSERT TO anon, authenticated WITH CHECK (true)';
        EXECUTE 'CREATE POLICY "video_submissions_update_all" ON video_submissions FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true)';
        EXECUTE 'CREATE POLICY "video_submissions_delete_all" ON video_submissions FOR DELETE TO anon, authenticated USING (true)';
    END IF;
END $$;
...
-- ALTER TABLE video_submissions DISABLE ROW LEVEL SECURITY;
