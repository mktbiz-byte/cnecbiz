...
const TaxInvoiceRequestsTab = () => {
  ...
  const handleManualIssue = async () => {
    ...
    try {
      const response = await fetch('/.netlify/functions/issue-tax-invoice-manual', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          invoiceData: {
          ...
          amount: amount,
          itemName: manualForm.itemName.trim() || '포인트 충전',
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-start">
                  <AlertCircle className="w-5 h-5 text-yellow-600 mt-0.5 mr-3 flex-shrink-0" />
                  <div>
                    <h4 className="text-sm font-medium text-yellow-800">수동 발행 안내</h4>
                    <p className="text-sm text-yellow-700 mt-1">
                      충전 신청 없이 직접 세금계산서를 발행합니다. 발행 후 취소할 수 없으니 정보를 정확히 입력해주세요.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {manualForm.companyName && manualForm.amount && (
                <div className="bg-blue-50 rounded-lg p-4">
                  <h4 className="font-semibold text-blue-900 mb-3">발행 미리보기</h4>
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="font-bold text-blue-700">{parseInt(manualForm.amount.replace(/,/g, '') || 0).toLocaleString()}원</div>
                  </div>
                </div>
              )}
            </div>
