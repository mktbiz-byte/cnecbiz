  ...
  const generateAIGuideFromData = async (campaignData) => {
    ...
    try {
      ...
      if (campaignData.required_hashtags && campaignData.required_hashtags.length > 0) {
        ...
        // 트렌드 분석: 중간 복잡도 → gemini-2.5-flash-lite (4K RPM, 무제한 RPD)
...
` : ''
      ...
      // 가이드 리뷰 생성: 복잡한 콘텐츠 → gemini-2.5-flash-lite (품질 중요)
...
JSON만 응답하세요.`
  ...
  const regenerateAIGuide = async () => {
    ...
    try {
      ...
      // 가이드 재생성: 복잡한 콘텐츠 → gemini-2.5-flash-lite (품질 중요)
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent?key=${apiKey}`,
        {
          method: 'POST',
          ...
          <div className="space-y-6">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4 flex items-start gap-3">
              ...
              <div className="text-sm text-yellow-800">
                <p className="font-semibold mb-1">⚠️ 원하시는 스타일의 컴셉을 꼭 확인해 주세요.</p>
                <p>체크박스를 통해 원하지 않는 컴셉을 선택 해제할 수 있습니다.</p>
              </div>
            </div>
