...
const CampaignCreationKorea = () => {
  ...
  const [campaignForm, setCampaignForm] = useState({
    campaign_type: 'planned',  // 'planned', 'oliveyoung', '4week_challenge', 'story_short', 'threads_post', 'x_post'
    ...
    try {
      ...
      if (data) {
        ...
        setCampaignForm({
          ...
          // SNS 업로드 예정일
          step1_sns_deadline: formatDate(data.step1_sns_deadline),
          step2_sns_deadline: formatDate(data.step2_sns_deadline),
  ...
  const handleDownloadContract = () => {
    ...
    <ol><li>크리에이터가 제작한 콘텐츠의 1차 저작권은 크리에이터에게 귀속됩니다.</li><li>광고주는 캠페인 계약 범위 내에서 콘텐츠를 활용할 수 있으며, 계약 범위를 초과하는 2차 활용은 별도 동의가 필요합니다.</li><li>2차 활용 기간은 크리에이터의 SNS 업로드일로부터 1년입니다.</li><li>2차 활용 시 콘텐츠의 변형, 재편집은 원저작자의 인격권을 침해하지 않는 범위 내에서 가능합니다.</li><li><strong>2차 활용 기간 만료 후, 크리에이터의 SNS 게시물은 삭제 의무 없이 유지될 수 있으나, 해당 콘텐츠를 Meta(Facebook/Instagram) 광고 소재로 사용하는 것은 불가합니다.</strong> 기간 만료 후 Meta 광고에 크리에이터 콘텐츠를 활용하려면 별도의 2차 활용 계약이 필요합니다.</li></ol>
  ...
  const handleSubmit = async (e) => {
    ...
    // 스토리 숏폼/스레드/X는 카테고리 자동 설정 → 검증 스킵
    if (!['story_short', 'threads_post', 'x_post'].includes(campaignForm.campaign_type)) {
    ...
    // 스레드/X 전용 검증
    if (['threads_post', 'x_post'].includes(campaignForm.campaign_type)) {
      const tg = campaignForm.text_content_guide || {}
      if (!tg.hook?.trim()) { setError('관심 유도(Hook) 가이드를 입력해주세요.'); return }
      if (!tg.value?.trim()) { setError('신뢰 구축(Value) 가이드를 입력해주세요.'); return }
  ...
  const executeSubmit = async () => {
    try {
      ...
      if (!['story_short', 'threads_post', 'x_post'].includes(campaignForm.campaign_type) && (!campaignForm.category || campaignForm.category.length === 0)) {
      ...
      const allowedFields = {
        ...
        // 영상 제출 마감일 및 SNS 업로드 예정일
        content_submission_deadline: campaignForm.start_date || null,  // 기획형: start_date = 영상 제출 마감일
        sns_upload_deadline: campaignForm.end_date || null,  // 기획형: end_date = SNS 업로드 예정일
        // 올리브영 SNS 업로드 예정일
        step1_sns_deadline: campaignForm.step1_sns_deadline || null,
        step2_sns_deadline: campaignForm.step2_sns_deadline || null,
        // 4주 챌린지 SNS 업로드 예정일
        ...
        // 공통 신규 필드
        reference_urls: campaignForm.reference_urls?.length ? campaignForm.reference_urls : [],
        // 스레드/X 전용 필드
        ...((['threads_post', 'x_post'].includes(campaignForm.campaign_type)) ? {
          text_content_guide: campaignForm.text_content_guide || null,
          target_region: 'KR'
        } : {}),
        // 스토리 숏폼 전용 필드
        ...(campaignForm.campaign_type === 'story_short' ? {
          ...
          story_content_guide: campaignForm.story_content_guide || null,
          cnec_shop_utm_link: campaignForm.cnec_shop_utm_link || null,
          target_region: 'KR'
        } : {})
      }
      ...
      const titleSuffixMap = { 'story_short': '스토리', 'threads_post': '스레드', 'x_post': 'X' }
      const titleSuffix = titleSuffixMap[campaignForm.campaign_type] || categoryNames
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <div className="bg-gray-50 pt-16 pb-6 lg:pt-8 lg:pb-8 lg:py-12">
        ...
        <div className="text-center mb-6 lg:mb-8">
          <h1 className="text-xl lg:text-3xl font-bold text-gray-900 mb-2">뷰티 브랜드를 위한 6가지 캠페인</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 max-w-7xl mx-auto">
            ...
            <div
              ...
              onClick={() => setCampaignForm(prev => ({
                ...prev,
                campaign_type: 'story_short',
                is_oliveyoung_sale: false,
                category: ['instagram'],
                total_slots: prev.total_slots || 10,
                remaining_slots: prev.total_slots || 10
              }))}
            >
              {/* NEW 배지 */}
              <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                <span className="bg-teal-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
                  NEW
                </span>
              </div>
              ...
              <ul className="space-y-2 text-sm flex-1">
                <li className="flex items-center gap-2">
                  ...
                  <span>최소 10초 이상 스토리 영상</span>
                </li>
                <li className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-teal-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                  <span>클린본 + 스크린샷 제출</span>
                </li>
                <li className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-teal-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                  <span>수정 불가 (추가 시 +2만원)</span>
                </li>
                <li className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-teal-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                  <span>2차 활용 포함</span>
            ...
            >
              ...
              <div className="mb-4 pt-2">
                <h3 className="text-base font-bold text-gray-900 mb-1">스레드 포스트</h3>
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl lg:text-3xl font-bold text-gray-900">₩20,000</span>
                  <span className="text-gray-500 text-sm">/건</span>
                </div>
                <p className="text-gray-500 text-xs mt-2 leading-relaxed">Threads 텍스트 기반 콘텐츠로 진정성 있는 제품 리뷰와 바이럴을 만듭니다</p>
              </div>
              ...
              <ul className="space-y-2 text-sm flex-1">
                <li className="flex items-center gap-2">
                  ...
                  <span>3단계 퍼널 (Hook→Value→Offer)</span>
                </li>
                <li className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-blue-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                  <span>포스트 URL + 스크린샷 제출</span>
                </li>
                <li className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-blue-500 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                  <span>#광고 표시 필수</span>
            ...
            >
              ...
              <div className="mb-4 pt-2">
                <h3 className="text-base font-bold text-gray-900 mb-1">X 포스트</h3>
                <div className="flex items-baseline gap-1">
                  <span className="text-2xl lg:text-3xl font-bold text-gray-900">₩20,000</span>
                  <span className="text-gray-500 text-sm">/건</span>
                </div>
                <p className="text-gray-500 text-xs mt-2 leading-relaxed">X(구 트위터) 텍스트 포스트로 빠른 확산과 인용 기반 바이럴을 유도합니다</p>
              </div>
              ...
              <ul className="space-y-2 text-sm flex-1">
                <li className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-gray-800 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                  <span>3단계 퍼널 (Hook→Value→Offer)</span>
                </li>
                <li className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-gray-800 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                ...
                <li className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-gray-800 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                  <span>#광고 / #ad 표시 필수</span>
                </li>
                <li className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-gray-800 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                  <span>임프레션 / 북마크 추적</span>
                </li>
              </ul>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
                ...
                <div className="lg:col-span-2 space-y-4 lg:space-y-8">
                  ...
                  <div className="bg-white rounded-2xl p-4 lg:p-8 shadow-sm border border-gray-100">
                    <h3 className="text-base lg:text-lg font-bold text-gray-900 flex items-center gap-2 mb-2">
                  ...
                  <div className="flex gap-4 lg:hidden">
                    <Button
                      type="submit"
                      disabled={processing || (campaignForm.campaign_type !== 'story_short' && campaignForm.category.length === 0)}
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  <div>
                                    <span className="text-xs text-gray-500">영상 제출 마감</span>
                                    <Input
                                      type="date"
                                  ...
                                  <div>
                                    <span className="text-xs text-gray-500">SNS 업로드 예정</span>
                                    <Input
                                      type="date"
            ...
            {campaignForm.campaign_type === '4week_challenge' && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
                ...
                <div className="lg:col-span-2 space-y-4 lg:space-y-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl border border-purple-200">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <Label htmlFor="week1_deadline" className="text-purple-700 font-semibold">Week 1 *</Label>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div>
                            ...
                            <Input
                              type="date"
                              value={campaignForm.week1_sns_deadline}
                              onChange={(e) => setCampaignForm(prev => ({ ...prev, week1_sns_deadline: e.target.value }))}
                              className="mt-1 bg-white"
                            />
                          </div>
                        </div>
                        <p className="text-xs text-purple-600 mt-2">Week 1 날짜 설정 시 Week 2~4 마감일 자동 계산 (+7일씩)</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                          ...
                          <div className="space-y-2">
                            <div>
                              <span className="text-xs text-gray-500">영상 제출</span>
                              <Input
                                type="date"
                            ...
                            <div>
                              <span className="text-xs text-gray-500">SNS 업로드</span>
                              <Input
                                type="date"
                                value={campaignForm.week2_sns_deadline}
                                onChange={(e) => setCampaignForm(prev => ({ ...prev, week2_sns_deadline: e.target.value }))}
                                className="h-9 text-sm mt-1"
                              />
                            </div>
                          </div>
                        ...
                        <div className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                          ...
                          <div className="space-y-2">
                            ...
                            <div>
                              ...
                              <Input
                                type="date"
                                value={campaignForm.week3_sns_deadline}
                                onChange={(e) => setCampaignForm(prev => ({ ...prev, week3_sns_deadline: e.target.value }))}
                                className="h-9 text-sm mt-1"
                              />
                            </div>
                          </div>
                        ...
                        <div className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                          ...
                          <div className="space-y-2">
                            ...
                            <div>
                              ...
                              <Input
                                type="date"
                                value={campaignForm.week4_sns_deadline}
                                onChange={(e) => setCampaignForm(prev => ({ ...prev, week4_sns_deadline: e.target.value }))}
                                className="h-9 text-sm mt-1"
                              />
                            </div>
                          </div>
                  ...
                  {/* 상품 정보 입력 */}
                  <div className="bg-white rounded-2xl p-4 lg:p-8 shadow-sm border border-gray-100">
                    <h3 className="text-base lg:text-lg font-bold text-gray-900 flex items-center gap-2 mb-4">
                      <span className="w-8 h-8 bg-gradient-to-br from-amber-500 to-orange-600 rounded-lg flex items-center justify-center">
                        <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
                      </span>
                      상품 정보 입력
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
                ...
                <div className="lg:col-span-2 space-y-4 lg:space-y-8">
                  ...
                  <div className="bg-white rounded-2xl p-4 lg:p-8 shadow-sm">
                    ...
                    <div className="space-y-6">
                      <div ref={brandInfoRef}>
                        <Label className="text-sm font-semibold text-gray-700 mb-2 block">브랜드명 *</Label>
                        <Input
                          value={campaignForm.brand}
                          onChange={e => setCampaignForm(prev => ({ ...prev, brand: e.target.value }))}
                      ...
                      {/* 제품명 */}
                      <div>
                        <Label className="text-sm font-semibold text-gray-700 mb-2 block">제품명 *</Label>
                        <Input
                          value={campaignForm.product_name}
                          onChange={e => setCampaignForm(prev => ({ ...prev, product_name: e.target.value }))}
                      ...
                      {/* 썸네일 이미지 */}
                      <div>
                        <Label className="text-sm font-semibold text-gray-700 mb-2 block">캠페인 썸네일</Label>
                        <div className="flex items-center gap-4">
                          {campaignForm.image_url && (
                            <img src={campaignForm.image_url} alt="썸네일" className="w-20 h-20 rounded-xl object-cover border" />
                          )}
                          <div>
                            <input
                              ref={thumbnailInputRef}
                              ...
                              onChange={async (e) => {
                                ...
                                try {
                                  const fileExt = file.name.split('.').pop()
                                  const fileName = `campaign_thumbnail_${Date.now()}.${fileExt}`
                                  const client = supabaseKorea || supabaseBiz
                                  const { error: uploadError } = await client.storage
                                    .from('campaign-images')
                                    .upload(fileName, file)
                  ...
                  <CnecShopLinkGenerator
                    ...
                    discountCode={campaignForm.discount_code}
                    campaignId={null}
                    onChange={(updates) => setCampaignForm(prev => ({ ...prev, ...updates }))}
                  />

                  {/* 레퍼런스 URL (신규) */}
                  <div className="bg-white rounded-2xl p-4 lg:p-8 shadow-sm">
                    <ReferenceUrlsInput
                      urls={campaignForm.reference_urls || []}
                      onChange={(urls) => setCampaignForm(prev => ({ ...prev, reference_urls: urls }))}
                    />
                  </div>

                  {/* 상품 안내 (고정) */}
                  <div className="bg-white rounded-2xl p-4 lg:p-8 shadow-sm">
                    <h2 className="text-lg lg:text-xl font-bold text-gray-900 mb-4">상품 안내</h2>
                    <div className="space-y-3">
                      <div className="flex items-start gap-3 p-3 bg-teal-50 rounded-xl">
                        <svg className="w-5 h-5 text-teal-500 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                        <div>
                          <span className="text-sm font-medium text-gray-900">영상 길이</span>
                          <p className="text-xs text-gray-600 mt-0.5">최소 10초 이상</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-amber-50 rounded-xl">
                        ...
                        <div>
                          <span className="text-sm font-medium text-gray-900">수정 불가</span>
                          <p className="text-xs text-gray-600 mt-0.5">수정 요청 시 20,000원 추가 과금</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-xl">
                        <svg className="w-5 h-5 text-blue-500 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                        <div>
                          <span className="text-sm font-medium text-gray-900">2차 활용</span>
                          <p className="text-xs text-gray-600 mt-0.5">크리에이터 동의 하에 광고 소재 등 재활용 가능</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-purple-50 rounded-xl">
                        ...
                        <div>
                          <span className="text-sm font-medium text-gray-900">제출물</span>
                          <p className="text-xs text-gray-600 mt-0.5">클린본 영상 + 스토리 스크린샷 (광고코드 불필요)</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                        <svg className="w-5 h-5 text-gray-500 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                        <div>
                          <span className="text-sm font-medium text-gray-900">대상 리전</span>
                          <p className="text-xs text-gray-600 mt-0.5">한국</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-1">
                  <div className="sticky top-6">
                    <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 text-white shadow-xl">
                      <h3 className="text-lg font-bold mb-6">견적 요약</h3>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-medium">{campaignForm.total_slots}명</span>
                        </div>
                      ...
                      <div className="mt-6 space-y-3">
                        <Button
                          ...
                          form="campaign-form"
                          disabled={processing}
                          className="w-full py-4 bg-teal-500 hover:bg-teal-600 text-white font-bold text-base rounded-xl"
                        >
                          {processing ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : null}
                          {processing ? '저장 중...' : (editId ? '수정하기' : '캠페인 개설하기')}
                        </Button>
                        <Button type="button" variant="outline" onClick={() => navigate('/company/campaigns')} className="w-full py-4 bg-transparent border-slate-600 text-gray-300 hover:bg-slate-800">
                          취소
                        </Button>
                ...
                <div className="lg:col-span-2 space-y-4 lg:space-y-8">
                  ...
                  <div className="bg-white rounded-2xl p-4 lg:p-8 shadow-sm">
                    <h2 className="text-lg lg:text-2xl font-bold text-gray-900 mb-2">
                      {campaignForm.campaign_type === 'threads_post' ? '스레드' : 'X'} 포스트 캠페인 설정
                    </h2>
                    <p className="text-gray-500 mb-6">텍스트 기반 콘텐츠 가이드를 설정하고 캠페인을 개설합니다.</p>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="border-t pt-6">
                        <TextContentGuideForm
                          guide={campaignForm.text_content_guide || {}}
                          onChange={(guide) => setCampaignForm(prev => ({ ...prev, text_content_guide: guide }))}
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          <Label className="text-sm font-semibold text-gray-700 mb-2 block">포스트 업로드 마감일 *</Label>
                          <Input
                            type="date"
                            value={campaignForm.end_date}
                            onChange={e => setCampaignForm(prev => ({ ...prev, end_date: e.target.value }))}
                  ...
                  <div className="sticky top-6">
                    <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 text-white shadow-xl">
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center">
                          <span className="text-gray-400 text-sm">상품</span>
                          <span className="font-medium">{campaignForm.campaign_type === 'threads_post' ? '스레드 포스트' : 'X 포스트'}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-gray-400 text-sm">단가</span>
                          <span className="font-medium">₩20,000/건</span>
                      ...
                      <div className="mt-6 space-y-3">
                        <Button
                          ...
                          form="campaign-form"
                          disabled={processing}
                          className="w-full py-4 bg-blue-500 hover:bg-blue-600 text-white font-bold text-base rounded-xl"
                        >
                          {processing ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : null}
                          {processing ? '저장 중...' : (editId ? '수정하기' : '캠페인 개설하기')}
                        </Button>
